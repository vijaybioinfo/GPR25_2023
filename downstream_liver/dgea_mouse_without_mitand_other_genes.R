...
# if(grepl("3.6.1", getRversion())) .libPaths('/home/kmlanderos/R/x86_64-pc-linux-gnu-library/3.6')
...
optlist <- list(
  ...
  optparse::make_option(
    opt_str = c("--genesfilt"), type = "character", default = 'none',
    help = paste0("pattern to delete genes")
  )
)
...
# opt$method<-"mastlog2cpm"
# opt$metadata<-"/mnt/bioadhoc-temp/Groups/vd-vijay/fcastaneda/rnaseq-sc-standar/gpr25/lung08/results/clustering/gpr25_lung_08_xdoublets_clean1_clusters_noCD8n_cellcycleREGRESSION/.object_meta.data_seurat_mean0.01_pct25_pc20.rds"
# opt$expression_data<-"/mnt/bioadhoc-temp/Groups/vd-vijay/fcastaneda/rnaseq-sc-standar/gpr25/lung08/results/clustering/gpr25_lung_08_xdoublets_clean1_clusters_noCD8n_cellcycleREGRESSION/.object_stem_seurat_mean0.01_pct25.rds"
# opt$group1<-"JuPa08_Mo_KO_CD8_1H"
# opt$group2<-"JuPa08_Mo_WT_CD8_1H"
# opt$newnames<-"JuPa08_Mo_KO_CD8_1HvsJuPa08_Mo_WT_CD8_1H_EMMA"
# opt$hname<-"orig.lib"
# opt$output_dir<-"/mnt/bioadhoc-temp/Groups/vd-vijay/fcastaneda/rnaseq-sc-standar/gpr25/lung08/results/dgea/groups_differences_lung08_exp3"
# opt$filters<-"list()"
# opt$context<-"KOvsWT"
# opt$covariates<-"none"
# opt$down_sample<-FALSE
# opt$genesetf<-"none"
# opt$ctrans<-"log2cpm"
# opt$padj_threshold<-0.05
# opt$fc_threshold<-0.25
# opt$annotation<-
# opt$thresh_min<-0
# opt$add_df<-"none"
# opt$verbose<-TRUE
library(edgeR)
 opt$genesfilt<- c("^Rpl\\d", "^Rps\\d", "^Gm\\d\\d", "^mt-", "AY036118") #c("^RPL\\d", "^RPS\\d", "^MT-")
 genesfilt<-opt$genesfilt
 cat(genesfilt)
...
if(opt$verbose){
  cat(cyan(    "-----------------------------------------\n"))
  cat(red_bold("- Differential gene expression analysis EMMA MODIFIED (ELIMINATES MT AND RSP, RPL) -\n"))
  cat(cyan(    "-----------------------------------------\n"))
}
...
cat(genesfilt, "Eliminating genes \n")
if(all(genesfilt != "none")){ res <- res[!grepl(paste(genesfilt, collapse="|"), res$gene),] }
